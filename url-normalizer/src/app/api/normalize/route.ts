...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    raw = raw.trim();
    if (raw.length > 8_000) return NextResponse.json({ error: "URL too long (max 8k chars)" }, { status: 413 });
    if (!/^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(raw)) raw = "https://" + raw;
    let parsed: URL;
    try { parsed = new URL(raw); } catch { return NextResponse.json({ error: "Invalid URL" }, { status: 400 }); }
    ...
    if (sortQuery) {
      ...
      Array.from(params.keys()).sort().forEach((k) => params.getAll(k).forEach((v) => sorted.append(k, v)));
      for (const k of Array.from(params.keys())) params.delete(k);
      sorted.forEach((v, k) => params.append(k, v));
    }
    if (forceHttps && (parsed.protocol === "http:" || parsed.protocol === "https:")) parsed.protocol = "https:";
    if (lowercaseHost) parsed.hostname = parsed.hostname.toLowerCase();
    if (removeFragment) parsed.hash = "";
    ...
    return NextResponse.json({
      ...
      meta: { trackingRemoved, changed, original: raw },
    });
  } catch {
