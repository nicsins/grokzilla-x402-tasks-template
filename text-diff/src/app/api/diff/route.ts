...
/**
 * x402 Text Diff – No-human-in-loop microservice
 * Price guidance: 0.005 – 0.015 USDC per call
 *
 * Accepts: { "a": string, "b": string, "options"?: { "mode": "lines"|"words"|"chars", "context": number, "format": "structured"|"unified" } }
 * Returns: { "success": true, "diff": ..., "meta": {...} }
 */
...
function simpleDiff(aTokens: string[], bTokens: string[]): Op[] {
  // Classic LCS-based minimal diff (O(n*m) but fine for micro payloads)
...
function toUnified(ops: Op[], context = 2): string {
  // Very lightweight unified-style output
  const lines: string[] = [];
  let buffer: Op[] = [];
  const flush = () => {
  ...
  for (const op of ops) {
    if (op.op === "equal") {
      ...
      if (buffer.length > context * 2) {
        // keep only trailing context
        buffer = buffer.slice(-context);
      }
    } else {
      flush();
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (a.length + b.length > 200_000) {
      return NextResponse.json({ error: "Combined input too large (max ~200k chars)" }, { status: 413 });
    }

    const options = body.options || {};
    const mode = (["lines", "words", "chars"].includes(options.mode) ? options.mode : "lines") as
      | "lines"
      | "words"
      | "chars";
    ...
    const ops = simpleDiff(aTokens, bTokens);

    let adds = 0,
      removes = 0;
    for (const o of ops) {
      if (o.op === "insert") adds++;
      if (o.op === "delete") removes++;
    }

    const result =
      format === "unified"
        ? toUnified(ops, context)
        : ops.map((o) => ({ op: o.op, text: o.text }));
    ...
    return NextResponse.json({
      ...
      meta: {
        adds,
        removes,
        equals: ops.length - adds - removes,
        mode,
        format,
        aLength: a.length,
        bLength: b.length,
