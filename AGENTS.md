...
3. Prefer deterministic endpoints (all three templates here are).
4. Keep payloads token-efficient. These services return compact JSON.

## Current Services (2026-08-17)
...
All accept a special `x-test-payment: true` header for local testing only. Never use in production.

## Pricing Philosophy

Micro-transactions. Agents should be able to call these dozens or hundreds of times without friction.
